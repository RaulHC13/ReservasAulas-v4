...
public class ControladorAnularReserva {
	...
	@FXML private void anularReserva() {
		Reserva reserva = null;
		
		try {
			reserva = getReserva();
			if (reserva == null) {
				Dialogos.mostrarDialogoError("ERROR", "ERROR: No se ha podido anular la reserva.");
			} else {
				controladorPrincipal.anularReserva(reserva);
				Dialogos.mostrarDialogoInformacion(null, "Se ha anulado la reserva.");	
			}
	...
	private Reserva getReserva() {
		...
		if (aula != null && permanencia != null) {
			try {
			reserva = Reserva.getReservaFicticia(aula, permanencia);
			} catch (NullPointerException e) {
			}
		}
			return reserva;
